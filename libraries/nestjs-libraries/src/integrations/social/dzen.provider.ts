...
export class DzenProvider extends PlaywrightAbstract implements SocialProvider {
  identifier = 'dzen';
  name = 'Дзен';
  ...
  async authenticate(params: {
    ...
    try {
      ...
      if (!isLoggedIn) return 'Dzen login failed — check Yandex credentials';
      await page.close();
      ...
      await page.close();
      await this.closeBrowser();
      return `Dzen auth error: ${err.message}`;
    }
  }
  ...
  async checkLoginStatus(page: Page): Promise<boolean> {
    try {
      await page.goto('https://dzen.ru/profile/editor/publications', {
        waitUntil: 'domcontentloaded',
        timeout: 15000,
      });
      return !page.url().includes('passport.yandex');
    } catch {
      return false;
    }
  }
  ...
  async performLogin(page: Page, credentials: string): Promise<void> {
    ...
    await this.humanType(page, '#passp-field-login', credentials);
  ...
  async uploadVideo(
    ...
    try {
      await page.goto('https://dzen.ru/profile/editor/video-upload', {
        waitUntil: 'domcontentloaded',
        timeout: 30000,
      });
      await this.humanDelay();
      ...
      const titleInput = await page.$(
        '[data-testid="title-input"], input[placeholder*="заголовок"], input[name="title"]'
      );
      if (titleInput) {
      ...
      const descInput = await page.$(
        'textarea[placeholder*="описание"], textarea[name="description"]'
      );
      if (descInput) {
      ...
      return { success: false, error: `Dzen upload failed: ${err.message}` };
    }
  }
}
